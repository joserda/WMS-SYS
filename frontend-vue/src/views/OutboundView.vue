...
import { getProducts, getWarehouses, getLocations, createOutboundOrder, getOutboundOrders, type Warehouse, type Location } from '@/api'
...
const handleSubmit = async () => {
  if (!customerName.value.trim()) { ElMessage.warning('请输入客户名称'); return }
  if (rows.value.length === 0) { ElMessage.warning('请添加至少一条出库明细'); return }
  ...
  <div class="outbound-page">
    <el-card shadow="never">
      ...
      <el-tabs v-model="activeTab" @tab-change="onTabChange">
        <el-tab-pane label="创建出库单" name="create">
          <el-form label-width="100px" @submit.prevent>
            <el-form-item label="客户名称" required>
              <el-input v-model="customerName" placeholder="请输入客户名称" style="max-width: 400px" clearable />
            </el-form-item>
          </el-form>
          ...
          <el-empty v-if="rows.length === 0" description="暂无出库明细，请点击「添加明细」" :image-size="100" />
          <div class="submit-bar">
            <el-button type="danger" size="large" :loading="submitting" :disabled="rows.length === 0" @click="handleSubmit">提交出库单</el-button>
          </div>
        </el-tab-pane>
